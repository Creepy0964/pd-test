...
export const entryWizard = new Scenes.WizardScene<MyContext>(
  ...
  async (ctx) => {
    await ctx.editMessageText(
      `прежде чем приступить к тесту\\, учти\\: нажимая на кнопку ниже\\, ты даешь свое согласие на сбор анонимной статистики и последующую ее использование для публикаций в [новостном канале бота](https://t.me/heeheeatry)\n\nсобираются только анонимные ID результатов\\, состояние ментального здоровья\\, сырые баллы и подсчитанный окончательный результат`,
      {
        reply_markup: {
